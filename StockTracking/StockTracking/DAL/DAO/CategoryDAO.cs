...
namespace StockTracking.DAL.DAO
{
    public class CategoryDAO : StockContext, IDAO<CATEGORY, CategoryDetailDTO>
    {
        ...
        public List<CategoryDetailDTO> Select()
        {
            ...
            var list = db.CATEGORies.Where(x => x.isDeleted == false).ToList(); // Exclude deleted records
        ...
        {
            List<CategoryDetailDTO> categories = new List<CategoryDetailDTO>();
            var list = db.CATEGORies.Where(x => x.isDeleted == isDeleted).ToList(); // Include deleted records only
            foreach (var item in list)
            {
                CategoryDetailDTO dto = new CategoryDetailDTO();
                dto.ID = item.ID;
